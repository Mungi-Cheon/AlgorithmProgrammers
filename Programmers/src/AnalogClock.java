...
public class AnalogClock {
    ...
    public int solution(int h1, int m1, int s1, int h2, int m2, int s2){
        int answer = 0;
        // 시, 분을 초로 변환
        int start_sec = _toSec(h1,m1,s1);
        int end_sec = _toSec(h2, m2, s2);

        answer = _cntAlarm(end_sec) - _cntAlarm(start_sec); 
        // TODO : 시,분,초침이 다 겹쳤을 경우 카운트
        return answer;
    }

    private int _toSec(int h, int m, int s){
        // 1분 = 60초, 1시간 = 60분 = 3600초, 12시간 = 720분 = 43,200초
        return (h * 60 * 60) + (m * 60) + s;
    }

    private int _cntAlarm(int sec){
        int cnt = 0;
        int h_cnt = sec * 719 / 43200; // 60초마다 시침과 초침이 겹치므로 12시간 = 719회 (60초부터 1회 count)
        int m_cnt = sec * 59 / 3600; // 60초마다 분침과 초침이 겹침. 60분 = 59회 (60초부터 1회 count)
        cnt = h_cnt + m_cnt;
        return cnt;
    }
    ...
    //         sum_ms = (plus_M + plus_S / 60); // 분과 초를 합산 
    //         tmp_M =  sum_ms % 60; // 60분인 경우 tmp_M은 0, 그외의 경우 1~59
    //         tmp_H = (plus_H + sum_ms / 60) % 24; // 24시인 경우 0, 그외의 경우 1~23
